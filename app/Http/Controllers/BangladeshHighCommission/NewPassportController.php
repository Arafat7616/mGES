...
class NewPassportController extends Controller
{
    public function request(){
        $newPassports = NewPassportService::where('service_status','Forwarded')
                     ->orderBy('id','DESC')->get();
        return view('BangladeshHighCommission.newPassport.request', compact('newPassports'));
    }

    public function approved(){
        $newPassports = NewPassportService::where('service_status','Approved')
                     ->orderBy('id','DESC')->get();
        return view('BangladeshHighCommission.newPassport.approved', compact('newPassports'));
    }

    public function rejected(){
        $newPassports = NewPassportService::where('service_status','Rejected')
                     ->orderBy('id','DESC')->get();
        return view('BangladeshHighCommission.newPassport.rejected', compact('newPassports'));
    }

    public function status($new_passport_service_id){
        $newPassport = NewPassportService::FindOrFail($new_passport_service_id);
        return view('BangladeshHighCommission.newPassport.status', compact('newPassport'));
    }
