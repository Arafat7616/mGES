...
class MalaysianEmployerDashboardController extends Controller
{
    public function dashboard(){
        $user = User::find(Auth::user()->id);
        if ($user->active_status == "New" || $user->active_status == 'Pending' || $user->active_status == 'Rejected') {
            return view('MalaysianEmployer.pending-profile', compact('user'));
        }
        return view('MalaysianEmployer.welcome');
    }

    public function companyPrfileView(){
        $user = User::find(Auth::user()->id);
        return view('MalaysianEmployer.profile', compact('user'));
    }

    public function editProfile()
    {
        $user = Auth::user();
        return view('MalaysianEmployer.profile-edit', compact('user'));
    }
