...
class MalaysiaRecruitingAgencyDashboardController extends Controller
{
    public function dashboard()
    {
        $user = User::find(Auth::user()->id);
        if ($user->active_status == "New" || $user->active_status == 'Pending' || $user->active_status == 'Rejected') {
            return view('MalaysiaRecruitingAgency.pending-profile', compact('user'));
        }
        return view('MalaysiaRecruitingAgency.welcome');
    }

    public function companyPrfileView(){
        $user = User::find(Auth::user()->id);
        return view('MalaysiaRecruitingAgency.profile', compact('user'));
    }

    public function editProfile()
    {
        $user = Auth::user();
        return view('MalaysiaRecruitingAgency.profile-edit', compact('user'));
    }
