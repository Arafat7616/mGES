...
class TravelAgencyDashboardController extends Controller
{
    public function dashboard(){
        // return Auth::user()->id;
        $user = User::find(Auth::user()->id);
        if ($user->active_status != 'Approved') {
            return view('TravelAgency.pending-profile', compact('user'));
        }

        return view('TravelAgency.welcome');
    }
    public function companyPrfileView(){

        $user = User::find(Auth::user()->id);
        return view('TravelAgency.profile', compact('user'));
    }

    public function editProfile()
    {
        $user = Auth::user();
        return view('TravelAgency.profile-edit', compact('user'));
    }
