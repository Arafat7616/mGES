...
class OneStopServiceController extends Controller
{
    public function request(){
        $users = User::where('user_type','master-one-stop-service')->where('active_status', 'Pending')->orderBy('id','DESC')->get();
        return view('BangladeshAdmin.oneStopServices.request',compact('users'));
    }

    public function approved(){
        $users = User::where('user_type','master-one-stop-service')->where('active_status', 'Approved')->orderBy('id','DESC')->get();
        return view('BangladeshAdmin.oneStopServices.approved',compact('users'));
    }

    public function rejected(){
        $users = User::where('user_type','master-one-stop-service')->where('active_status', 'Rejected')->orderBy('id','DESC')->get();
