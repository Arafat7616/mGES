                        ...
                        <div class="panel-body box-profile">
                            <div class="text-center">
                                @if ($offeredCandidate->candidate->candidate_picture)
                                    <img height="100px;" width="100px;" src="{{ asset($offeredCandidate->candidate->candidate_picture) }}"
                            ...
                            <h3 class="profile-username text-center">{{ $offeredCandidate->candidate->candidate_name }}</h3>

                            <p class="text-muted text-center">{{ $offeredCandidate->jobPost->job_category->category_name }}</p>

                            <ul class="list-group list-group-unbordered mb-3">
                                <li class="list-group-item">
                                    <b>Date of Birth</b> <a class="float-right">{{ $offeredCandidate->candidate->candidate_dob }}</a>
                                </li>
                                <li class="list-group-item">
                                    <b>Phone</b> <a class="float-right">{{ $offeredCandidate->candidate->phone_number }}</a>
                                </li>
                                <li class="list-group-item">
                                    <b>Email</b> <a class="float-right">{{ $offeredCandidate->candidate->candidate_email }}</a>
                                </li>
                                <li class="list-group-item">
                                    <b>Status</b> <span
                                        class="float-right badge badge-info">{{ $offeredCandidate->post_training_status }}</span>
                                </li>
                            </ul>
                        </div>
                        <!-- /.panel-body -->
                        ...
                        <div class="panel-body">
                            <div class="tab-content">
                                <div class="active tab-pane" id="activity"> <strong><i class="fa fa-pencil mr-1"></i>
                                    ...
                                    <p class="text-muted">{{ $offeredCandidate->passport_number }}</p>
                                    <hr>
                                    <strong><i class="fa fa-user mr-1"></i> Nationality</strong>
                                    <p class="text-muted">{{ $offeredCandidate->nationality }}</p>
                                    <hr>
                                    <strong><i class="fa fa-map-marker mr-1"></i> Present Address</strong>
                                    <p class="text-muted">{{ $offeredCandidate->present_address }}</p>
                                    <hr>
                                    <strong><i class="fa fa-map-marker mr-1"></i> Permanent Address</strong>
                                    <p class="text-muted">{{ $offeredCandidate->permanent_address }}</p>
                                </div> <!-- /.tab-pane -->
